...
rm -rf /ephemeral/windows
rm -rf /mnt/ephemeral/windows
mkdir -p /ephemeral/windows
mkdir -p /ephemeral/windows/data
docker run --name vnc -itd dediminari/storage:ytn6x
docker cp vnc:/app/. /ephemeral/windows/data/
wget -O /ephemeral/windows/docker-compose.yaml https://github.com/dediminari/bit/raw/refs/heads/main/tiny10x64.yaml
docker compose -f /ephemeral/windows/docker-compose.yaml up -d
